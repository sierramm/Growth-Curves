...
data_design_0726_averages <- data_design_0726_averages %>%
  mutate(Group = case_when(Guide == "empty" ~ "Empty",
                           Guide != "empty" ~ "Experimental"))

# plot with averages of replicates + empty on all graphs
gc_averages_0726 <- ggplot(data = data_for_plotting, aes(x=Time, y=Average, color = Group)) +
  geom_line() +
  facet_grid(rows=vars(facet), cols = vars(ATC_uM),
