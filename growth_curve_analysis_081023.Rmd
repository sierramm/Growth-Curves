...
wide_data_0810 <- read_wides("081023_gc_data.csv") # data from plate 1
...
tidy_data_0810 <- trans_wide_to_tidy(
  wides = wide_data_0810,
  id_cols = c("file", "Time"))
...
data_design_0810_all <- merge_dfs(tidy_data_0810, design_0810)


# convert time to numeric
# hms() since data has hours, minutes, and seconds (use hm() for hours & minutes)
# set unit to be hours (defaults to seconds)
data_design_0810_all$Time <- time_length(hms(data_design_0810_all$Time), unit = "hour")
...
data_design_0810_all$Well <- factor(data_design_0810_all$Well,
                                       levels=paste(rep(LETTERS[1:8], each=12), 1:12, sep=""))
data_design_0810_all$Guide <- factor(data_design_0810_all$Guide,
                                        levels=c("17", "18", "19", "empty"))

data_design_0810_all$Gene <- factor(data_design_0810_all$Gene, levels = c("ftsZ", "dnaA", "fusA", "proS", "ftsB", "empty"))

data_design_0810_all$PAM <- factor(data_design_0810_all$PAM, levels = c("NGG", "NAN", "NNN", "empty"))

# omit NA wells (use to plot only experimental wells)
data_design_0810_filtered <- data_design_0810_all %>%
  filter(!is.na(Guide))
...
data_design_0810_averages <- data_design_0810_filtered %>%
...
ggplot(data = data_design_0810_all, aes(x=Time, y=Measurements, color = ATC_uM)) +
  geom_line() +
  facet_wrap(~Well, nrow = 8, ncol = 12)

ggplot(data = data_design_0810_averages, aes(x=Time, y=Average, color = ATC_uM)) +
...
ggplot(data = data_design_0810_filtered, aes(x=Time, y=Measurements, color = Replicate)) +
  geom_line() +
  facet_grid(rows = vars(Gene, PAM), cols = vars(Vector, ATC_uM)) +
  theme_bw() +
  labs(title = "fusA Growth Assay", caption = "From Aug 10, 2023")


fusA_plot_0810 <- ggplot(data = data_design_0810_averages %>% filter(ATC_uM != 0.1), aes(x=Time, y=Average, color = ATC_uM)) +
...
ggsave("fusA_plot_0810.jpg", dpi = 700, height = 10, width = 16)
